...
try
    load(fullfile(load_path_with_artifact, 'phase.mat'), 'iFreq');
    load(fullfile(load_path_with_artifact, 'Mask.mat'), 'Mask');
    iFreq_with_artifact = iFreq;
...
try
    load(fullfile(load_path_with_motion, 'phase.mat'), 'iFreq');
...
% データサイズの確認
if ~(isequal(size(iFreq_no_motion), size(iFreq_with_artifact)) && isequal(size(iFreq_no_motion), size(iFreq_with_motion)))
...
iFreq_with_artifact_masked = iFreq_with_artifact;
iFreq_with_artifact_masked(Mask_with_artifact == 0) = NaN;
...
for slice_idx = 1:total_slices
    ...
    % Figure全体にタイトルを追加
    sgtitle(['3D Unwrapped Phase (iFreq) Comparison - Slice ' num2str(slice_idx)]);

    
    % --- 1. 左側: 体動なし (No Motion) ---
    ax1 = subplot(rows, cols, 1);
    
    data1 = iFreq_no_motion_masked(:, :, slice_idx);
    mesh(ax1, data1);
    ...
    data2 = iFreq_with_artifact_masked(:, :, slice_idx);
    mesh(ax2, data2);
    ...
    data3 = iFreq_with_motion_masked(:, :, slice_idx);
    mesh(ax3, data3);
