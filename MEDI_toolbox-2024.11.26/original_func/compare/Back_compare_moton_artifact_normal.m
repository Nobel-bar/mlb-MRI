...
try
    ...
    load(fullfile(load_path_no_motion, 'phase.mat'), 'iFreq'); 
    load(fullfile(load_path_no_motion, 'Mask.mat'), 'Mask');
    Back_no_motion = iFreq - RDF;
    Mask_no_motion = Mask;
    clear RDF Mask;
    fprintf('読み込み完了。\n');
catch ME
...
% データサイズの確認
if ~(isequal(size(Back_no_motion), size(Back_with_artifact)) && isequal(size(Back_no_motion), size(Back_with_motion)))
    warning('読み込んだ3つのデータセットのサイズが異なります。続行できません。');
    return;
end

total_slices = size(Back_no_motion, 3); % Z軸の総スライス数を取得
...
Back_no_motion_masked = Back_no_motion;
Back_no_motion_masked(Mask_no_motion == 0) = NaN;

Back_with_artifact_masked = Back_with_artifact;
Back_with_artifact_masked(Mask_with_artifact == 0) = NaN;

Back_with_motion_masked = Back_with_motion;
Back_with_motion_masked(Mask_with_motion == 0) = NaN;
...
for slice_idx = 1:total_slices
    ...
    % Figure全体にタイトルを追加
    sgtitle(['3D Back Comparison - Slice ' num2str(slice_idx)]);

    
    % --- 1. 左側: 体動なし (No Motion) ---
    ax1 = subplot(rows, cols, 1);
    
    data1 = Back_no_motion_masked(:, :, slice_idx);
    mesh(ax1, data1);
    ...
    data2 = Back_with_artifact_masked(:, :, slice_idx);
    mesh(ax2, data2);
    ...
    data3 = Back_with_motion_masked(:, :, slice_idx);
    mesh(ax3, data3);
