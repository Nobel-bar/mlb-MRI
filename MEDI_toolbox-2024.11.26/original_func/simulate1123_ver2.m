...
%% --- 5. k空間シミュレーション (並列化 & 高速化・修正版) ---
fprintf('\n5. k空間シミュレーション (parfor使用・修正版)...\n');
tic; % 計測開始
...
for slice_idx = 1 : num_slices
    ...
    % parfor用のループ回数
    num_ky = length(ky_collect_indices);
    num_kx = length(kx_collect_indices);
    
    % ★修正点: parfor内で書き込むための一時配列を用意 (サイズ: kx数 x ky数)
    simulated_block = complex(zeros(num_kx, num_ky));
    ...
    parfor i = 1:num_ky
        ...
        % ★修正点: インデックスにループ変数 'i' を直接使うことでエラー回避
        simulated_block(:, i) = k_line_signal;
    end
    
    % ★ループ後にまとめて書き戻す
    temp_k_space_slice(kx_collect_indices, ky_collect_indices) = simulated_block;
...
function k_space_line_signal = simulate_ky_line_fast(image_data, background_phase, ky_line_index, kx_collect_indices)
    ...
    % --- 座標定義 ---
    % k空間座標
    k_x_vals = (kx_collect_indices(:) - 1) - floor(Nx/2); % [M x 1]
    k_y_val  = (ky_line_index - 1) - floor(Ny/2);         % scalar
    
    % 実空間座標ベクトル
    x_vec = (((0:Nx-1) - floor(Nx/2)) / Nx).'; % [Nx x 1] (列ベクトル)
    y_vec = (((0:Ny-1) - floor(Ny/2)) / Ny);   % [1 x Ny] (行ベクトル)
    
    % --- 信号計算 (行列演算) ---
    % S(kx, ky) = sum_x [ exp(-i 2pi kx x) * { sum_y rho(x,y) * exp(-i 2pi ky y) } ]
    
    % 1. 実空間画像に誤差を付与
    rho_eff = image_data .* background_phase; % [Nx x Ny]
    
    % 2. Y方向の積分 (内積)
    % term_y = sum(rho(x,y) * exp(-i...)), size [Nx x 1]
    % exp_ky_vec: [Ny x 1] (転置して掛けるため)
    exp_ky_vec = exp(-1i * 2 * pi * k_y_val * y_vec).'; 
    
    % 行列積: [Nx x Ny] * [Ny x 1] -> [Nx x 1]
    % これで各x行ごとのy方向積分が一発で完了
    term_y = rho_eff * exp_ky_vec; 
    
    % 3. X方向の積分 (必要なkxのみ計算)
    % k_x_vals: [M x 1]
    % x_vec: [Nx x 1]
    % We want output [M x 1]. 
    % Kernel matrix E: [M x Nx], where E(m, n) = exp(-i 2pi kx_m * x_n)
    
    % 行列積用のカーネル作成 (bsxfun的な展開)
    % exponent: [M x Nx]
    exponent = -1i * 2 * pi * (k_x_vals * x_vec.'); 
    E_kx = exp(exponent);
    
    % 最終計算: [M x Nx] * [Nx x 1] -> [M x 1]
