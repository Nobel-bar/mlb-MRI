...
%
% [★ user のリクエストにより、セクション4.4 を `convn` (線形畳み込み) に変更 ★]
% [★ 警告: これは物理的に正しくなく、計算が非常に遅くなります ★]
%==================================================================================================
...
% --- 4.4. 回転後実空間データの合成 ---
% [★ user のリクエストに基づき、`.*` (乗算) を `convn` (線形畳み込み) に変更 ★]
% [★ 警告: この処理は非常に低速で、物理的シミュレーションとして正しくありません ★]
fprintf('   4.4. 実空間で 3D 線形畳み込み (convn) を実行中 (非常に低速)...\n');
% space_rotated = extend_high_rotated .* extend_back; % [★元の処理]
space_rotated = convn(extend_high_rotated, extend_back, 'same'); % [★変更後の処理]
fprintf('   ... 3D 線形畳み込み 完了。\n');
