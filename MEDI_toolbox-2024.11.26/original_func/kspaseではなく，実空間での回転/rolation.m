...
save_path = fullfile(load_base_path, image_file_4);
...
for slice_idx = 1:num_slices
    ...
    % % imrotateは複素数に直接対応していないため、実数部と虚数部を別々に回転
    % rotated_img_Re = imrotate(real(padded_img), theta, 'bilinear', 'crop');
    % rotated_img_Im = imrotate(imag(padded_img), theta, 'bilinear', 'crop');
    % 
    % % 再び複素数画像に結合
    % rotated_padded_img = rotated_img_Re + 1i * rotated_img_Im;

        % --- 1. 回転の定義 ---　より簡潔にするため．MATLABのImage Processing Toolbox
    theta = -18.6; % 元のコードと同じ回転角度
    tform = affine2d([cosd(theta)  -sind(theta)  0; ...
                      sind(theta)   cosd(theta)  0; ...
                      0             0            1]);
    
    % --- 2. 出力画像の参照情報 (サイズと位置を固定) ---
    % 'crop' と同じ動作にするため、入力画像と同じサイズ・位置の参照オブジェクトを作成
    output_view = imref2d(size(padded_img));
    
    % --- 3. 複素数画像の回転 ---
    % imwarpは複素数配列 padded_img を直接受け取ることができる
    rotated_padded_img = imwarp(padded_img, tform, 'bilinear', ...
                                'OutputView', output_view);

% これで、実数部と虚数部に分ける処理 (4行) が 1行で完了します。
...
% --- 4. 中央の切り出し (この後の処理は同じ) ---
    ...
    % --- 実数部 (Real) ---
    filename_Re = fullfile(save_path, ['Re_', base_filename]);
    fin1 = fopen(filename_Re, 'w');
    fwrite(fin1, real(artifact_img), 'double');
    fclose(fin1);

    % --- 虚数部 (Imaginary) ---
    filename_Im = fullfile(save_path, ['Im_', base_filename]);
    fin2 = fopen(filename_Im, 'w');
    fwrite(fin2, imag(artifact_img), 'double');
    fclose(fin2);

    % --- 強度 (Magnitude) ---
    filename_mag = fullfile(save_path, ['mag_', base_filename]);
    fin3 = fopen(filename_mag, 'w');
    fwrite(fin3, abs(artifact_img), 'double');
    fclose(fin3);

    % --- 位相 (Phase) ---
    filename_pha = fullfile(save_path, ['pha_', base_filename]);
    fin4 = fopen(filename_pha, 'w');
    fwrite(fin4, angle(artifact_img), 'double');
    fclose(fin4);
