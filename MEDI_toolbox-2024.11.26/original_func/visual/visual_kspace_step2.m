...
radius_steps = round(linspace(max_radius / num_steps_total, max_radius, num_steps_total));
radius_steps = unique(radius_steps); % 重複ステップを削除
num_steps_actual = length(radius_steps); % 実際のステップ数

% 表示するステップを選択
step_indices_to_show = [5, num_steps_actual]; % ステップ5と最終ステップ(25付近)

% 1x2のタイルレイアウトを持つFigureを準備
figure('Name', 'kスペース再構成比較 (Step 5 vs Final Step)', 'Position', [100, 100, 1000, 500]);
t_images = tiledlayout(1, 2, 'TileSpacing', 'compact', 'Padding', 'compact');
title(t_images, 'kスペース中心からのデータ追加による画像再構成', 'FontSize', 16, 'FontWeight', 'bold');

% (オプション) マスク自体の形状を表示するFigure
figure('Name', '使用したkスペース領域 (マスク)', 'Position', [1150, 100, 1000, 500]);
t_masks = tiledlayout(1, 2, 'TileSpacing', 'compact', 'Padding', 'compact');
title(t_masks, '使用したkスペース領域 (マスク)', 'FontSize', 16, 'FontWeight', 'bold');

% 選択したステップごとに処理
for i = 1:length(step_indices_to_show)
    
    step_index = step_indices_to_show(i);
    
    % ステップインデックスが有効範囲内かチェック
    if step_index > num_steps_actual
        fprintf('警告: ステップ %d は計算されたステップ数 %d を超えています。最終ステップを使用します。\n', step_index, num_steps_actual);
        step_index = num_steps_actual;
    end
    
    current_radius = radius_steps(step_index);
    ...
    title_str = sprintf('Step %d/%d (Mask: %dpx)', step_index, num_steps_actual, current_radius*2 + 1);
    title(title_str);
    axis off;
    ...
    nexttile(i);
    imshow(mask, []);
    title(sprintf('Step %d/%d', step_index, num_steps_actual));
    axis off;
