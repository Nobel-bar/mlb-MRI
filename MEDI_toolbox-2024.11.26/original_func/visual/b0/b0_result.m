...
% 
% 概要:
%   b0_map_processing.m で計算された、wrap適用前の磁場マップ (iFreq_raw) と
...
image_file_6 = '6_b0_data'; 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%変更あり%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
image_file_temp = 'F:\hamaguchi\20251204\3D_RSSG';  % <--- ' ' に変更
image_file_temp = 'F:\hamaguchi\20251204\2DSE';  % <--- ' ' に変更
image_file_0 = image_file_temp;
% 読み込みパスと保存パスを定義
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%変更あり%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%変更あり%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 読み込みパスと保存パスを定義
load_base_path = fullfile(image_file_0, image_file_6);

load(fullfile(load_base_path, 'B0_Map_Result.mat'));
...
% --- 統計量の計算 ---
val_max = max(abs(iFreq(:))); % 絶対値の最大値を取得

% --- 単位の自動判定 (閾値を20として判定) ---
if val_max < 20
    unit_str = 'ppm';
    disp('判定結果: 単位は [ppm] です');
else
    unit_str = 'Hz';
    disp('判定結果: 単位は [Hz] です');
end
...
h = colorbar; % カラーバーのハンドルを取得
ylabel(h, unit_str, 'FontSize', 12, 'FontWeight', 'bold'); % ★ここに単位を表示
...
xlabel('X Index');
ylabel('Y Index');
zlabel(unit_str, 'FontSize', 14, 'FontWeight', 'bold'); % ★Z軸に単位を表示
title('Before Wrapping (iFreq_raw)');
colorbar;
