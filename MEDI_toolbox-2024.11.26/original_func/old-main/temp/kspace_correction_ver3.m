...
% 【改変版: 3段階 階段状ROI置換 + 実画像/k空間 比較表示】
%==================================================================================================

fprintf('スクリプトを開始します (画像＆k空間比較版)\n');
clear variables;
close all;
...
% correct_RDF_small = RDF(1:DS_FACTOR:end, 1:DS_FACTOR:end, :); % 未使用のためコメントアウト
% correct_iFreq_small = iFreq(1:DS_FACTOR:end, 1:DS_FACTOR:end, :); % 未使用のためコメントアウト
...
%% --- 5. & 6. シミュレーション処理 ---
% (※ここでは可視化用スクリプトとして、Step 5, 6のループ計算は省略し、
%    単純回転画像を比較する Step 7 に重点を置きます。
%    もしハイブリッドシミュレーション結果と比較したい場合は、
%    前回の完全版コードのStep 5, 6を含めてください)
...
figure('Name', 'Image & K-Space Difference Comparison', 'WindowState', 'maximized');
colormap(gray(256));

% --- 上段: 実画像 (Image) の差分 ---
subplot(2, 4, 1); imshow(Diff_Img_0, [0, max_diff_img]);
title({'[Image Diff]', '0 deg vs Correct'}); colorbar;

subplot(2, 4, 2); imshow(Diff_Img_1, [0, max_diff_img]);
title({'[Image Diff]', '\theta/3 deg vs Correct'}); colorbar;

subplot(2, 4, 3); imshow(Diff_Img_2, [0, max_diff_img]);
title({'[Image Diff]', '2\theta/3 deg vs Correct'}); colorbar;

subplot(2, 4, 4); imshow(Diff_Img_3, [0, max_diff_img]);
title({'[Image Diff]', '\theta deg vs Correct'}); colorbar;


% --- 下段: k空間 (K-Space) の差分 ---
subplot(2, 4, 5); imshow(Diff_K_0, [0, max_diff_k]);
title({'[K-Space Log Diff]', '0 deg vs Correct'}); colorbar;

subplot(2, 4, 6); imshow(Diff_K_1, [0, max_diff_k]);
title({'[K-Space Log Diff]', '\theta/3 deg vs Correct'}); colorbar;

subplot(2, 4, 7); imshow(Diff_K_2, [0, max_diff_k]);
title({'[K-Space Log Diff]', '2\theta/3 deg vs Correct'}); colorbar;

subplot(2, 4, 8); imshow(Diff_K_3, [0, max_diff_k]);
title({'[K-Space Log Diff]', '\theta deg vs Correct'}); colorbar;

sgtitle(sprintf('Magnitude Difference (Black=Match, White=Mismatch)\nSlice: %d, Theta: %.1f', target_slice, theta), 'Interpreter', 'none');


%% --- ローカル関数 ---
% (必要に応じて元のスクリプトのStep5,6用の関数も残してください)
