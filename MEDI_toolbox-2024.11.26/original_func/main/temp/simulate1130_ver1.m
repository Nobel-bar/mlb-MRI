...
pix_start_row = 118; % k空間ROIの何行目から置き換えるか (1-based)
pix_start_col = 100; % Y列目 (X行目に関しては、この列以降を入れ替える) % 最大　352 
% width = 112; % 置き換える行数
...
% ダウンサンプリングに合わせて開始位置も調整
pix_start_row = round(pix_start_row / DS_FACTOR);
pix_start_col = round(pix_start_col / DS_FACTOR);
if pix_start_row < 1, pix_start_row = 1; end
if pix_start_col < 1, pix_start_col = 1; end

% --- 体動パラメータ ---
MOTION_PE_ERROR_MAX_RAD = 0; % 体動を入れる場合は値を設定
...
motion_start_line_shifted = x_start_cut + pix_start_row - 1;
...
for slice_idx = 1 : num_slices
    ...
    img_slice = extend_org_rotated(:,:,slice_idx);
...
% 比較用リファレンスは全領域を置換
base_k_space_direct(kx_collect_indices, ky_collect_indices, :) = k_space_direct_rotated(kx_collect_indices, ky_collect_indices, :);

% 1. 「X行目」の処理: Y列目以降のみ置換
target_row_idx = x_start_cut + pix_start_row - 1; % 絶対座標
target_col_start = y_start_org_cut + pix_start_col - 1; % 絶対座標(開始)
target_col_range = target_col_start : y_end_org_cut; % Y列目～最後

% 2. 「X行目以降」の処理: すべての列(ROI全幅)を置換
remaining_rows_start = target_row_idx + 1;
remaining_rows_range = remaining_rows_start : x_end_cut;
all_cols_range = ky_collect_indices; % ROIの全列

if remaining_rows_start <= x_end_cut
    base_k_space_artifact(remaining_rows_range, all_cols_range, :) = k_space_artifact(remaining_rows_range, all_cols_range, :);
end

% デバッグ出力
fprintf('Step 1 (Row %d): Col %d to End\n', target_row_idx, target_col_start);
fprintf('Step 2 (Row %d to %d): All Cols\n', remaining_rows_start, x_end_cut);
