...
function k_space_line_signal = simulate_ky_line_fast(image_data, background_phase, ky_line_index, kx_collect_indices)
    % image_data 実空間画像
    % background_phase 実空間の位相画像
    [Nx, Ny] = size(image_data);
    rho_eff = image_data .* background_phase; 
    
    % ky_line_indexは，置き換える位相エンコードの列番号　1列 kspace上の番号
    % kx_collect_indices 収集する周波数エンコードの行の範囲　?行分　　kspace上の番号
    ...
    % --- 4. 信号計算 (行列演算) ---
    % S(kx, ky) = sum_x sum_y rho(x,y) * exp(-i 2pi (kx*x + ky*y))  %% これが本来の計算
