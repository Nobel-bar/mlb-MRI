...
%% --- k空間の点を1つずつ実空間画像（縞模様）にするスクリプト ---
% 前提: workspaceに 'simulate_space' や 各indices、matrixサイズ等の変数が残っていること

% 保存先フォルダ
output_basis_dir = fullfile(save_path, 'kspace_points_visualized');
if ~exist(output_basis_dir, 'dir'), mkdir(output_basis_dir); end

fprintf('k空間の点ごとの画像化を開始します...\n');

% カウンター（全てやると終わらないため、数枚で止める用）
count = 0;
max_output = 5; % ★ここを書き換えると、出力する枚数を変更できます

% 全体のサイズでゼロのk空間を用意するひな形
template_k = complex(zeros(matrix_x, magnification, num_slices));

% --- ループ開始（指定範囲を走査） ---
% ※ 3重ループですが、max_output で途中で抜けます
% --- ループ開始 ---
for z_idx = hydrid_z_indices
    for y_idx = hydrid_y_indices
        for x_idx = hybrid_row_indices
            ...
            % 2. 1点だけ値をコピー
            target_val = simulate_space(x_idx, y_idx, z_idx);
            single_point_kspace(x_idx, y_idx, z_idx) = target_val;
            
            % 3. 逆フーリエ変換
            img_basis_3d = ifftn(ifftshift(single_point_kspace));
            img_basis_slice = img_basis_3d(:, :, z_idx);
            
            % 4. 表示
            f = figure;
            set(f, 'Position', [100, 100, 1000, 500]);
            
            % --- 変更点: abs ではなく real を表示 ---
            subplot(1, 2, 1);
            imagesc(real(img_basis_slice)); % <--- ここを real に変更！
            colormap gray; axis image; axis off;
            title(sprintf('Point(%d,%d,%d) - Real Part\n(Wave Pattern)', x_idx, y_idx, z_idx));
            
            subplot(1, 2, 2);
            imagesc(angle(img_basis_slice));
            colormap hsv; axis image; axis off;
            title('Phase (Wave Direction)');
            
            % --- 重要: 描画を強制更新 ---
            drawnow; 
            
            % 保存する場合（必要ならコメントアウトを外す）
            % filename = sprintf('Point_X%d_Y%d_Z%d.png', x_idx, y_idx, z_idx);
            % saveas(f, fullfile(output_basis_dir, filename));
            % close(f); % 保存後に閉じるなら有効化
            
        end
        if count > max_output, break; end
    end
    if count > max_output, break; end
end
fprintf('処理終了。\n');
