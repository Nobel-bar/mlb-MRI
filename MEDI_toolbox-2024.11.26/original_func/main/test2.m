...
ll = ifftn(ifftshift(kk));
simulate = fftshift(fftn(ll.* extend_back_rotated)); 
base_and_simulate_space(hybrid_row_indices, hydrid_y_indices, hydrid_z_indices) = simulate(hybrid_row_indices, hydrid_y_indices, hydrid_z_indices);
...
% ファイル保存
filename_base = sprintf(['g_artifact_th%.1f'], theta);
