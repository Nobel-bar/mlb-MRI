% LBV.m
%   [fL, interior] = LBV(iFreq,Mask,matrix_size,voxel_size,tol,depth,peel,N1,N2,N3)
% 
%   output
%   fL - the local field
%   interior - the interior on which the field is available (this is equal
%              to Mask with peel+1 layers peeled off ( +1 because that is 
%              the boundary layer)
% 
%   input
...
function [fL, interior] = LBV(iFreq,Mask,matrix_size,voxel_size,tol,depth,peel,N1,N2,N3)
    if (nargin <5)
        tol = 0.01;
    end
    if (nargin <6)
    ...
    % c++ interface
    [tmp, interior] = mexMGv6(fT,mask,matrix_size,voxel_size,tol,depth,peel,N1,N2,N3);
    fL = reshape(tmp,matrix_size);
    interior = reshape(typecast(interior, class(Mask)), matrix_size);
