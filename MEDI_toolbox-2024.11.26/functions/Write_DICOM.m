...
xa20_flag=-1;
xa20_fields={'OtherPatientIDsSequence', 'OriginalAttributesSequence'};
xa20_remove={};
...
for slice=1:size(M,3)
    if load_flag
        ...
        if (xa20_flag<0)
            try
                version=info.SoftwareVersions;
            catch
                version=info.SoftwareVersion;
            end
            if strcmp(version(end-3:end),'XA20')
                xa20_remove = xa20_fields(isfield(info, xa20_fields));
                xa20_flag = ~isempty(xa20_remove);
            else
                xa20_flag = 0;
            end
        end
        if (xa20_flag)
            info=rmfield(info,xa20_remove(isfield(info, xa20_remove)));
        end
