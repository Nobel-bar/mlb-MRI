...
for i = 1:length(filelist)
    ...
    if (info2.ImageType(18)=='P')||(info2.ImageType(18)=='p')
        ...
        fprintf(progress);
        RescaleSlope = nan; RescaleIntercept = nan;
        if isfield(info2, 'RescaleSlope'); RescaleSlope = info2.RescaleSlope; end
        if isfield(info2, 'RescaleIntercept'); RescaleIntercept = info2.RescaleIntercept; end
        ...
        if any(isnan([RescaleSlope RescaleIntercept])) 
            [RescaleSlope, RescaleIntercept] = guess_phase_scale_intercept(ph);
        end
        iPhase(:,:,rctr)  = (ph*RescaleSlope + RescaleIntercept)/single(max(ph(:)))*pi;%phase
...
if nargin<2
    tags={'SliceLocation','ImagePositionPatient',...
        'EchoTime','EchoNumber','ImageType','RescaleSlope','RescaleIntercept','ParallelAcquisitionTechnique'};
end
...
function [scale, intercept] = guess_phase_scale_intercept(A)
    amin=min(A(:));
    amax=max(A(:));
    if (amin>=0)
        pmax=2^ceil(log2(amax));
        scale=2;
        intercept=-pmax;
    else
        scale=1;
