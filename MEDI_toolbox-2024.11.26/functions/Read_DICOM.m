...
% head in the sand methodology
ignore.errors={'images:dicominfo:fileVRDoesNotMatchDictionary'};
ignore.num=numel(ignore.errors);

for i=1:ignore.num
    ignore.warning{i} = warning('query',ignore.errors{i});
    warning('off',ignore.warning{i}.identifier);
end
...
switch lower(manufacturer)
    ...
    case {'siemens','siemens healthcare gmbh','siemens healthineers'}
        ...
        if isfield(info, 'PerFrameFunctionalGroupsSequence')
            [iField,voxel_size,matrix_size,CF,delta_TE,TE,B0_dir,files]=Read_DICOM_Siemens_3D(DicomFolder,filelist(i:end));
        else
...
for i=1:ignore.num
    warning(ignore.warning{i}.state,ignore.warning{i}.identifier);
end
